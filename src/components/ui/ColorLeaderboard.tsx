...
async function fetchColorScores(force = false) {
...
export function invalidateColorLeaderboard() {
  cache = null;
  void fetchColorScores(true);
}
...
export default function ColorLeaderboard() {
  ...
  const sync = useCallback((force = false) => {
    void fetchColorScores(force).then(() => {
      setData(cache);
      setLoading(false);
    });
  }, []);
