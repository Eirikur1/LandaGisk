...
const TILE_COUNT = 6;
const HELP_SEEN_KEY = "help-seen:flags";
...
function FlagGuesserInner() {
  ...
  const decoyCountry = useMemo(() => {
    const others = WORLD_COUNTRIES.filter((c) => c.code !== target.code);
    return others[seededIndex(day + "decoy", others.length)]!;
  }, [day, target]);
        ...
        <div className="w-full max-w-sm xl:max-w-lg flex flex-col flex-1">
          ...
          >
            ...
            >
              ...
              />
              {/* 3×2 cover tiles — no gaps/borders so nothing bleeds through */}
              <div
                className="absolute inset-0 grid pointer-events-none"
                style={{ gridTemplateColumns: "repeat(3, 1fr)", gridTemplateRows: "repeat(2, 1fr)", gap: 0 }}
              >
              ...
              {/* "Give it a guess" overlay — shown only before any guesses */}
              {guesses.length === 0 && (
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none gap-1">
                  <p className="text-sm font-semibold italic opacity-40 text-(--color-foreground)">
                    It&apos;s only a 1 in {WORLD_COUNTRIES.length} guess.
                  </p>
                  <p className="text-xs font-semibold italic opacity-30 text-(--color-foreground)">
                    Give it a go — is it {decoyCountry.name}?
                  </p>
                </div>
              )}
            </div>
          ...
          {(failed || gaveUp) && !won && (
            <motion.div
              ...
              className="mb-3 rounded-2xl border border-(--color-border) bg-(--color-surface) px-4 py-3 flex items-center justify-between gap-4"
            >
              <div>
