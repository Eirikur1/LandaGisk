...
export default function GridGuesser() {
  ...
  return (
    <div className="relative z-10 pt-0 pb-6 px-4 sm:px-8 flex flex-col items-center">
      {/* Title */}
      <div className="w-full mb-3">
        ...
        <motion.p className="text-sm text-(--color-muted) leading-relaxed mt-2" style={{ fontFamily: "var(--font-sans)" }}
        ...
        <motion.p className="text-[10px] tracking-[0.25em] text-(--color-muted) mt-1 opacity-80" style={{ fontFamily: "var(--font-sans)" }}
      ...
      {phase !== "intro" && (
        <div className="w-full max-w-2xl flex items-center gap-2 mb-2">
          ...
          {phase === "playing" && <SizeBadge label={cell.sizeLabel} />}
        </div>
      )}

      {/* Card: map on top, controls pinned to bottom */}
      <motion.div
        className="w-full max-w-2xl rounded-3xl overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.12)] flex flex-col"
      ...
      >
        {/* Map */}
        <div className="w-full overflow-hidden rounded-t-3xl" style={{ aspectRatio: "700/380" }}>
          <Suspense fallback={<div className="w-full h-full bg-[#c8dff0]" />}>
            {phase === "intro" ? (
              <IntroMap />
            ) : (
              <WorldMap
        ...
        <AnimatePresence mode="wait" initial={false}>
          {phase === "intro" && (
            <motion.div key="intro-ctrl"
              initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="px-5 py-4 flex items-center justify-center"
            >
              <button type="button" onClick={handlePlay}
                className="rounded-full px-8 py-2.5 text-sm font-bold text-white transition-opacity hover:opacity-90"
          ...
          {phase === "playing" && (
            <motion.div key={`play-ctrl-${round}`}
              initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="px-4 py-3 flex flex-col gap-2 border-t border-black/6"
            >
              <div className="flex items-center justify-between gap-3">
                <p className="text-xs" style={{ ...F, color: "#888" }}>
                  Found <span style={{ color: FIGMA_BLUE }}>{cell.found.length}</span> of {cell.targets.length}
                </p>
                <button type="button" onClick={giveUp}
                  className="text-[10px] tracking-[0.12em] uppercase hover:opacity-60 transition-opacity"
                  style={{ ...F, color: "#bbb" }}>
                  Give up →
                </button>
              </div>
              ...
              {cell.found.length > 0 && (
                <div className="flex flex-wrap gap-1">
                  {cell.found.map((c) => (
                    <span key={c.id} className="text-[10px] px-1.5 py-0.5 rounded-full text-white font-semibold"
          ...
          {phase === "result" && lastResult && (
            <motion.div key={`result-ctrl-${round}`}
              initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="px-4 py-3 flex flex-col gap-2 border-t border-black/6"
            >
              <div className="flex items-center justify-between gap-3">
                <div>
                  ...
                  <p className="text-3xl font-black leading-none tabular-nums" style={{ ...F, color: FIGMA_BLUE }}>
                    {lastResult.score.toFixed(0)}
                    <span className="text-base ml-1 font-semibold" style={{ color: "#bbb" }}>/100</span>
                  </p>
                </div>
                <button type="button" onClick={nextRound}
                  className="rounded-full px-6 py-2.5 text-sm font-bold text-white transition-opacity hover:opacity-90 shrink-0"
                  style={{ ...F, background: FIGMA_BLUE }}>
              ...
              <div>
                <p className="text-[10px] tracking-[0.2em] uppercase mb-1" style={{ ...F, color: "#aaa" }}>
...
function IntroMap() {
  return (
    <div className="w-full h-full bg-[#c8dff0] flex items-center justify-center">
