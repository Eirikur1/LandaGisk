-- Add 'color' to game_type constraint and color_xp to leaderboard view.
-- Run this in the Supabase SQL editor.

-- 1. Drop old check constraint and add new one including 'color'
alter table public.game_scores
  drop constraint if exists game_scores_game_type_check;

alter table public.game_scores
  add constraint game_scores_game_type_check
  check (game_type in ('world', 'waterfall', 'flags', 'mushroom', 'color'));

-- 2. Recreate leaderboard view with color_xp column
